...
import { BookOpen, Search, LogIn, LayoutDashboard, Download, Eye, FileText, HandHelping } from "lucide-react";
...
export default async function CatalogPage({
  ...
  const uniqueCategories = Array.from(new Set(categoriesData?.map(c => c.name).filter(Boolean)));
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 selection:bg-orange-200">
      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div>
            {!user ? (
              <Link href="/login" className="inline-flex items-center space-x-2 text-sm font-semibold text-orange-600 hover:text-orange-700 transition-colors">
                <LogIn className="w-5 h-5" />
                <span>Acceder</span>
              </Link>
            ) : (
              <Link href="/dashboard" className="inline-flex items-center space-x-2 text-sm font-semibold text-white bg-orange-600 px-5 py-2 rounded-xl hover:bg-orange-700 transition-all shadow-sm hover:-translate-y-0.5 hover:shadow-md">
      ...
      <section className="relative pt-16 pb-24 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          <h1 className="text-4xl md:text-5xl font-extrabold text-slate-900 tracking-tight mb-4">
            Catálogo <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-600 to-yellow-500">Digital</span>
          </h1>
          <p className="text-lg text-slate-600 max-w-2xl mx-auto mb-8">
            Encuentra libros físicos, trabajos de grado y documentos PDF descargables.
          </p>
          
          <form method="GET" action="/" className="max-w-2xl mx-auto relative group">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-24 relative z-20">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
          {books?.map((book) => (
            <div key={book.id} className="group flex flex-col bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300">
              ...
              <div className="p-5 flex flex-col flex-1 bg-white">
                <p className="text-xs font-bold tracking-wider text-orange-600 mb-1 uppercase">{book.category || 'General'}</p>
                <p className="text-sm text-slate-500 line-clamp-2 mb-4 font-medium">{book.authors || 'Autor desconocido'}</p>
                  ...
                  <div className="pt-3 border-t border-slate-100 flex flex-col gap-3">
                    ...
                    {book.ebooks_total > 0 && (
                      <div className="flex gap-2">
                        <Link href={`/read/${book.id}`} className="flex-1 flex items-center justify-center gap-1.5 bg-orange-50 hover:bg-orange-100 text-orange-700 px-3 py-2 rounded-lg text-xs font-bold transition-colors border border-orange-200">
                          <Eye className="w-3.5 h-3.5" /> Leer Online
                        </Link>
                        <Link href={`/read/${book.id}`} className="flex-1 flex items-center justify-center gap-1.5 bg-slate-50 hover:bg-slate-100 text-slate-700 px-3 py-2 rounded-lg text-xs font-bold transition-colors border border-slate-200" title="Ver Detalles y Descargar">
                    ...
                    {book.ebooks_total === 0 && book.is_reference && (
                      <button className="w-full flex items-center justify-center gap-1.5 bg-slate-50 text-slate-500 px-3 py-2 rounded-lg text-xs font-bold border border-slate-200 cursor-not-allowed">
                        <FileText className="w-3.5 h-3.5" /> Solo Consulta en Sala
                      </button>
                    )}
                  </div>
                </div>
        ...
        {(!books || books.length === 0) && (
          <div className="text-center py-24 bg-white rounded-3xl border border-slate-200 border-dashed">
            ...
            <p className="text-slate-500 mt-2">Intenta buscar con otros términos o cambia de categoría.</p>
          </div>
        )}
      </main>
    </div>
